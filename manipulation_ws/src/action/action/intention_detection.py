...
#         self.cam_intr = {
#             'l': (1060.09, 1059.09, 958.91, 561.57),
#             'r': (1059.97, 1059.97, 963.08, 522.35)
#         }

#         self.cam_extr = {
#             'l': {'euler': (-2.382, 0.473, -0.722), 'translation': [0.1126, -0.5019, 0.558]},
#             'r': {'euler': (-2.298, 0.299, 2.348), 'translation': [0.9037, 0.4392, 0.5986]}
#         }

#         # 初始化 Open3D 可视化器（非阻塞）
#         self.vis = o3d.visualization.Visualizer()
#         self.vis.create_window(window_name='ZED 实时点云 (优化)', width=1280, height=720)
#         self.vis_added = False
#         self.geo_map = {}

#         # 设置刷新定时器
#         self.create_timer(0.5, self.refresh_visualization)
...
#         self.get_logger().info("ZEDPointCloudNode (优化版) 已启动，等待相机数据...")
...
#                 # 实时展示图像
#                 cv2.imshow(f"RGB - {cam}", img)
#                 cv2.waitKey(1)

#                 self.try_generate_pointcloud(cam)
...
#     def try_generate_pointcloud(self, cam):
#         if not self.rgb_queues[cam] or not self.depth_queues[cam]:
#             return

#         if cam == 'l' and not self.has_l:
#             self.get_logger().info("[左相机] 数据接收成功 ✅")
#             self.has_l = True
#         if cam == 'r' and not self.has_r:
#             self.get_logger().info("[右相机] 数据接收成功 ✅")
#             self.has_r = True

#         rgb = self.rgb_queues[cam][-1]
#         depth = self.depth_queues[cam][-1]

#         fx, fy, cx, cy = self.cam_intr[cam]
#         roll, pitch, yaw = self.cam_extr[cam]['euler']
#         t = np.asarray(self.cam_extr[cam]['translation'])
...
#         mask = (z.reshape(-1) > 0) & (z.reshape(-1) < 5)
#         pts = pts[mask]
...
#         pcd = pcd.voxel_down_sample(0.01)  # 降采样提速

#         frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
#         frame.transform(T)

#         self.pcd_dict[cam] = pcd
#         self.frame_dict[cam] = frame

#     def refresh_visualization(self):
#         world = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2)
#         items = []

#         if self.has_l and self.has_r and self.pcd_dict['l'] and self.pcd_dict['r']:
#             if not self.icp_done:
#                 self.get_logger().info("🎉 双目数据齐全，执行 ICP 配准")
#                 voxel = 0.01
#                 pcd_l_d = self.pcd_dict['l'].voxel_down_sample(voxel)
#                 pcd_r_d = self.pcd_dict['r'].voxel_down_sample(voxel)

#                 pcd_l_d.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=voxel * 2, max_nn=30))
#                 pcd_r_d.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=voxel * 2, max_nn=30))

#                 reg = o3d.pipelines.registration.registration_icp(
#                     pcd_l_d, pcd_r_d, voxel * 2, np.eye(4),
#                     o3d.pipelines.registration.TransformationEstimationPointToPlane()
#                 )

#                 pcd_l_icp = copy.deepcopy(self.pcd_dict['l'])
#                 pcd_l_icp.transform(reg.transformation)

#                 frame_l_icp = copy.deepcopy(self.frame_dict['l'])
#                 frame_l_icp.transform(reg.transformation)

#                 self.geo_map = {
#                     'pcd_r': self.pcd_dict['r'],
#                     'pcd_l_icp': pcd_l_icp,
#                     'frame_r': self.frame_dict['r'],
#                     'frame_l_icp': frame_l_icp,
#                     'world': world
#                 }

#                 for g in self.geo_map.values():
...
#         elif self.has_l and self.pcd_dict['l']:
#             if 'pcd' not in self.geo_map:
#                 self.geo_map = {
#                     'pcd': self.pcd_dict['l'],
#                     'frame': self.frame_dict['l'],
#                     'world': world
#                 }
#                 for g in self.geo_map.values():
#                     self.vis.add_geometry(g)
#             else:
#                 self.geo_map['pcd'] = self.pcd_dict['l']
#                 self.geo_map['frame'] = self.frame_dict['l']
#                 for g in self.geo_map.values():
#                     self.vis.update_geometry(g)

#         elif self.has_r and self.pcd_dict['r']:
#             if 'pcd' not in self.geo_map:
#                 self.geo_map = {
#                     'pcd': self.pcd_dict['r'],
#                     'frame': self.frame_dict['r'],
#                     'world': world
#                 }
#                 for g in self.geo_map.values():
#                     self.vis.add_geometry(g)
#             else:
#                 self.geo_map['pcd'] = self.pcd_dict['r']
#                 self.geo_map['frame'] = self.frame_dict['r']
#                 for g in self.geo_map.values():
#                     self.vis.update_geometry(g)
...
def load_point_cloud(depth_path: str, rgb_path: str, intrinsics: tuple, extrinsics: dict):
    depth = np.load(depth_path)
    rgb = cv2.cvtColor(cv2.imread(rgb_path), cv2.COLOR_BGR2RGB)
...
class Intention():
    ...
    def process_hand_direction_from_ros_rgbd(self, rgb_msg, depth_msg, T_wc, camera_intrinsics):
        """
        ...
        if not (0 <= u1 < w and 0 <= v1 < h and 0 <= u2 < w and 0 <= v2 < h):
            print(f"⚠️ 食指坐标超出图像范围")
            return None, None 

        z1 = float(depth[v1, u1])
        z2 = float(depth[v2, u2])

        if z1 <= 0 or z2 <= 0 or z1 > 5.0 or z2 > 5.0:
        ...
        # 变换到世界坐标系
        p1_world = (T_wc @ p1_cam)[:3]
        p2_world = (T_wc @ p2_cam)[:3]
...
class PersistentOpen3DViewer:
    ...
    def load_and_register(self):
        ...
        pcd_left, frame_left = load_point_cloud('./background/l_depth.npy', './background/l_rgb.png', intr_left, extr_left)

        intr_right = (1059.9764, 1059.9764, 963.0756, 522.3530)
        extr_right = {'euler': (-2.29, 0.29, 2.34), 'translation': [0.90, 0.43, 0.59]}
        pcd_right, frame_right = load_point_cloud('./background/r_depth.npy', './background/r_rgb.png', intr_right, extr_right)
        ...
        while rclpy.ok():
            with self.lock:
                if self.left_camera_active and self.right_camera_active:
                    ...
                    # print(rgb_msg_l and rgb_msg_r and depth_msg_l and depth_msg_r)
                    # TODO
                    if rgb_msg_l is not None and depth_msg_l is not None and rgb_msg_r is not None and depth_msg_r is not None:
                        # print("0")
                        direction_l, origin_l = self.intention.process_hand_direction_from_ros_rgbd(rgb_msg_l, depth_msg_l, self.T_wc_l, self.intrinsics_l)
                        direction_r, origin_r = self.intention.process_hand_direction_from_ros_rgbd(rgb_msg_r, depth_msg_r, self.T_wc_r, self.intrinsics_r)
                        if direction_l is not None and origin_l is not None and direction_r is not None and origin_r is not None :
                            ...
                            if np.linalg.norm(direction) > 1e-6:
                                direction = direction / np.linalg.norm(direction)
                                self.viewer.update_arrow_async(direction, origin)
                    ...
                    # self.rgb_buffer['left'] = None
                    # self.depth_buffer['left'] = None
                    # self.rgb_buffer['right'] = None
                    # self.depth_buffer['right'] = None
                    ...
                    if rgb_msg is not None and depth_msg is not None:
                        direction, origin = self.intention.process_hand_direction_from_ros_rgbd(rgb_msg, depth_msg, self.T_wc_r, self.intrinsics_r)
                    ...
                    if rgb_msg is not None and depth_msg is not None:
                        direction, origin = self.intention.process_hand_direction_from_ros_rgbd(rgb_msg, depth_msg, self.T_wc_l, self.intrinsics_l)
                        if direction is not None and origin is not None:
                            if np.linalg.norm(direction) > 1e-6:
                                direction = direction / np.linalg.norm(direction)
                                self.viewer.update_arrow_async(direction, origin)
