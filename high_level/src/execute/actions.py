...
def execute_action_sequence(actions):
    ...
    for i, action in enumerate(actions):
        ...
        if act_type == "perceive":
            ...
            success, box_center_point, seg_center_point = VLM_agent(target, image_path="images/rgb.png")
            pixel_points = [seg_center_point] if seg_center_point is not None else [box_center_point]
            print(f"Perceived pixel points: {pixel_points}")
            depth_img = np.load('images/depth.npy')
            H, W = depth_img.shape
            rgb_img = cv2.cvtColor(cv2.imread('images/rgb.png'), cv2.COLOR_BGR2RGB)
            pixel_points = np.array(pixel_points, dtype=int)
            u = pixel_points[:, 0]
            v = pixel_points[:, 1]
            # 利用索引从深度图中提取深度值
            depths = depth_img[v, u]  # 注意顺序：行是 v，列是 u          
            target_point = pixel_points.tolist()  # 获取目标的像素点坐标
            world_points, _ = pixels_to_world_right(target_point, depths, rgb_img=rgb_img)
            if len(world_points) == 1:
                world_points = tuple(world_points[0])
            else:
                world_points = [tuple(point) for point in world_points]
                
            print(f"World point in left camera: {world_points}")
            time.sleep(5)
