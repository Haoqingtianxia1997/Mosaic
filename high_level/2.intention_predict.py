...
realtime video feed: human pose + head pose ROI + YOLO object detection + voice interaction
"""
import cv2
import time
...
def intention_predict(model_path) -> None:
    """
    Open camera, do real-time human pose, head pose, object detection and voice interaction
    """
    # Initialize transcriber and YOLOv8 model
    model_path = model_path if model_path else 'yolo_model/yolo11m.pt'  # Default weights path
    transcriber = VoiceTranscriber()
    model = YOLO(model_path)               # Custom trained weights
    label_times = {}                         # {label: last_seen_time}
    window_seconds = 15                       # Label retention window (seconds)
    tts_interval = 30                        # TTS/STT interval (seconds)
    detected_labels = set()                  # Detected labels in the current window
    last_query_result = None                 # Last STT query result
    ...
    while capture.isOpened():
        ...
        if not ret:                          # If read failed, exit
            break
        ...
        # If voice interaction is in progress, pause detection and show prompt
        ...
        # ---------- 2. Only process if pose landmarks are detected ----------
        if pose_results.pose_landmarks:
            ...
            # Count how many fall inside the body
            roi_length_check = 100            # Only take 40 cm (or pixels 400) for judgment
            samples = 40                      # Number of samples
            inside = 0                        # ← Reset every frame

            for i in range(1, samples + 1):   # (0, 1] uniform sampling
                ratio = i / samples
            ...
            # If more than 70% of the samples are inside the body → Skip YOLO
            ...
            # ---------- 7. Maintain Sliding Window for Labels ----------
            detected_labels = {
            ...
            # If you need LM analysis, you can use label_analyzer.analyze_labels
        ...
        if (current_time - last_tts_time > tts_interval) and detected_labels:
            ...
            # ✅ Write to transcription.txt (so the main thread can read it)
